...
function StepCaptureModal({ item, onClose, onStepsSaved }) {
  ...
  async function handleAIPolish() {
    ...
    setPolishing(true)
    try {
      const stepDescriptions = steps.map((s, i) => `Step ${i + 1}: ${s.narration || '(no description)'}`).join('\n')
...
Format it cleanly. If you're unsure about exact quantities, give reasonable estimates and mark them with "~" (approximate). Keep it practical and home-cook friendly.`
      ...
      // Use Supabase edge function path since polish-recipe is Supabase-only currently
      const { supabase } = await import('../lib/supabaseClient').catch(() => ({ supabase: null }))
      if (!supabase) throw new Error('AI polish requires Supabase. Set up edge functions.')
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center overflow-y-auto py-8">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl mx-4">
        ...
        <div className="p-6 space-y-6">
          ...
          >
            {uploadingStep !== null ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm font-medium text-gray-700">Paste screenshot (Cmd+V anywhere) or click to upload</p>
                <p className="text-xs text-gray-400">Take a screenshot while watching, then paste it here</p>
              </div>
            )}
          ...
          {steps.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-700">{steps.length} step{steps.length !== 1 ? 's' : ''} captured</h3>
                ...
                <div key={index} className="flex gap-3 bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Step {index + 1}</span>
                      <div className="flex-1" />
                      <button onClick={() => moveStep(index, -1)} disabled={index === 0} className="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-30" title="Move up"><GripVertical className="w-4 h-4 rotate-180" /></button>
                      <button onClick={() => moveStep(index, 1)} disabled={index === steps.length - 1} className="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-30" title="Move down"><GripVertical className="w-4 h-4" /></button>
                      <button onClick={() => removeStep(index)} className="p-1 text-red-400 hover:text-red-600" title="Remove step"><Trash2 className="w-4 h-4" /></button>
                    </div>
                    <textarea
          ...
          {steps.length > 0 && (
            <div className="border-t border-gray-100 pt-6">
              ...
              >
                {polishing ? <><Loader2 className="w-5 h-5 animate-spin" />Generating recipe...</> : <><Sparkles className="w-5 h-5" />Generate Recipe with AI</>}
              </button>
              {polishedRecipe && (
                <div className="mt-4 bg-violet-50 rounded-xl p-4 border border-violet-200">
                  ...
                  <div className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{polishedRecipe}</div>
                  <button onClick={promotePolishedRecipe} className="mt-4 flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition-colors">
                    Save as Full Recipe
                  </button>
                </div>
              )}
        ...
        <div className="flex justify-between items-center p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
          <p className="text-xs text-gray-400">{steps.length === 0 ? 'Paste or upload your first screenshot to begin' : `${steps.length} steps ready`}</p>
          <div className="flex gap-3">
            <button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition-colors">Cancel</button>
            <button onClick={saveSteps} disabled={saving || steps.length === 0} className="flex items-center gap-2 bg-amber-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-amber-700 transition-colors disabled:opacity-50">
