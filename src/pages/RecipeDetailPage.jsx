...
function RecipeDetailPage() {
  ...
  useEffect(() => {
    if (!listLoading && allRecipes.length > 0) {
      ...
      if (found) {
        ...
        if (Array.isArray(rawNotes)) setNotes(rawNotes)
        else if (rawNotes && typeof rawNotes === 'string') setNotes([{ date: null, text: rawNotes }])
        else setNotes([])
        setLoading(false)
        setError(null)
      } else if (!listLoading) {
  ...
  async function handleAutoTag() {
    ...
    setAutoTagging(true)
    try {
      const response = await fetch(AUTO_TAG_URL, {
        method: 'POST',
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-red-50 text-red-600 p-4 rounded-lg border border-red-200 max-w-md">
          ...
          <button onClick={() => navigate('/')} className="mt-4 block w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">
            Back to Recipes
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                isHidden ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-600 text-white hover:bg-gray-700'
              }`}
            >
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {editingImage && (
            <div className="p-4 bg-gray-50 border-t border-gray-200">
              ...
              <div className="flex gap-3">
                ...
                />
                <button onClick={() => setEditingImage(false)} className="bg-green-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-700 transition-colors">
                  Done
                </button>
                {imageUrl && (
                  <button onClick={() => { setImageUrl(''); setEditingImage(false) }} className="bg-red-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-red-700 transition-colors">
                    Remove
                  </button>
                )}
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              ...
              {CATEGORY_OPTIONS.map(option => (
                <button
                  ...
                  onClick={() => setCategory(prev =>
                    prev.includes(option.id) ? prev.filter(c => c !== option.id) : [...prev, option.id]
                  )}
          ...
          {sourceUrl && (
            <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-600 hover:text-blue-700 mb-4 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <select value={selectedFolder} onChange={(e) => setSelectedFolder(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white">
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setIsTried(true)} className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${isTried ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Tried</button>
                <button onClick={() => setIsTried(false)} className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${!isTried ? 'bg-gray-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Untried</button>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} onClick={() => { setRating(star); setIsTried(true) }} className={`text-4xl transition-all transform hover:scale-110 ${star <= rating ? 'text-yellow-400' : 'text-gray-300'} hover:text-yellow-400`}>★</button>
              ))}
              {rating > 0 && <button onClick={() => setRating(0)} className="ml-4 text-sm text-gray-600 hover:text-gray-800 underline">Clear rating</button>}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {DIETARY_OPTIONS.map(option => (
                <label key={option.id} className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={dietaryTags.includes(option.id)} onChange={() => toggleDietaryTag(option.id)} className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500" />
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <div className="prose max-w-none text-gray-700 whitespace-pre-wrap">{recipe.content || 'No recipe content available.'}</div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {!addingNote && (
              <button onClick={() => setAddingNote(true)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
          ...
          {addingNote && (
            <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="mt-3 flex items-center gap-3">
                <button
                  onClick={() => {
                    const entry = { date: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }), text: newNoteText.trim() }
                ...
                <button onClick={() => { setAddingNote(false); setNewNoteText('') }} className="px-5 py-2 text-gray-700 hover:bg-gray-100 rounded-lg text-sm font-medium transition-colors">
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {notes.map((note, idx) => (
                <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-gray-200 group">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-500">{note.date || 'Undated'}</span>
                    <button onClick={() => handleDeleteNote(idx)} className="p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all" title="Delete note">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
